...
class MCStructure(nn.Module):
    def __init__(self, param, inptnr, weight_num, bDec= True, b_perpoint_bias = True): #layer_info_lst= [(point_num, feature_dim)]
        ...
        if bDec:
            self.connection_layer_fn_lst = ["_unpool3","_unpool2","_unpool1"]#param.connection_layer_fn_lst_dec
        else:
            self.connection_layer_fn_lst = ["_pool1", "_pool2","_pool3"]#param.connection_layer_fn_lst_enc
        ...
        for l in np.arange(0,self.layer_num):           
            ...
            connection_info  = np.load("./VCMeshConv/connectmat/connections/body/"+self.connection_layer_fn_lst[l]+".npy")
            out_point_num = connection_info.shape[0]
...
class MCLoss(nn.Module):
    ...
    def compute_geometric_loss_l2(self, gt_pc, predict_pc,weights=[]):
        ...
        if(len(weights)==0):
            loss = (gt_pc-predict_pc).pow(2).mean()#.sum(2).mean()
