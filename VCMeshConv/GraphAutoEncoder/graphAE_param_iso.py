...
class Parameters():
    ...
    def read_config(self, fn):
        ...
        datainit=np.arange(8744)#np.arange(12273)
        datainit=datainit[:,np.newaxis]
        datainit=datainit.repeat(3,axis=1)
        self.initial_connection_fn =datainit#config.get("Params", "initial_connection_fn")
        data = datainit#np.load(self.initial_connection_fn)
        neighbor_id_dist_lstlst = data[:, 1:] # point_num*(1+2*neighbor_num) 
        self.point_num =8744#12273# 8744#12273
