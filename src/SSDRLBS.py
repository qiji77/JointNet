...
class SSDRLBS:
    def __init__(self, mesh_path, weights_path, trans_off_path, device):
        ...
        self.rest_pose_h = torch.hstack([self.v, torch.ones([self.v.shape[0], 1]).to(device).float()]). \
            reshape((-1, 4, 1))
        self.weights = torch.from_numpy(np.load(weights_path,allow_pickle=True)).to(device).t().float()
        '''a=torch.zeros(self.weights.size()).to(device)
        b=torch.ones(self.weights.size()).to(device)
        weight_temp=torch.where(self.weights>1,b,a)
        wt=weight_temp.sum(dim=1)
        print(self.weights.size())
        for i in range(12273):
            if wt[i]==0:
                value,index=torch.max(self.weights[i],dim=0)
                weight_temp[i,index]=1
        wt=weight_temp.sum(dim=1)
        for i in range(12273):
            if wt[i]==0:
                print("xxxx",i)
